        ...
        <div class="form-grid-3 mb-6">
            ...
            <div class="form-group">
                ...
                <input type="text" name="owner_contact" class="form-control"
                       value="{{ old('owner_contact', $business->owner_contact) }}">
            </div>
            <div class="form-group">
                ...
                <select name="owner_resident_id" class="form-control">
                    ...
                    @foreach($residents as $r)
                        <option value="{{ $r->id }}" {{ old('owner_resident_id', $business->owner_resident_id) == $r->id ? 'selected' : '' }}>
                            {{ $r->full_name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="form-grid-3 mb-6">
            <div class="form-group">
                ...
                <input type="date" name="permit_date" class="form-control"
                       value="{{ old('permit_date', $business->permit_date ? \Carbon\Carbon::parse($business->permit_date)->format('Y-m-d') : '') }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Expiry Date <span style="color:var(--crimson)">*</span></label>
                <input type="date" name="expiry_date" class="form-control"
                       value="{{ old('expiry_date', $business->expiry_date ? \Carbon\Carbon::parse($business->expiry_date)->format('Y-m-d') : '') }}" required>
            </div>
            <div class="form-group">
