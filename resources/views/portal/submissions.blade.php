...
<div class="portal-wrap-md">
    ...
    <div class="lookup-card">
        <form method="GET" action="{{ route('portal.submissions') }}" id="lookupForm">
    ...
    <div class="p-card">
        ...
        <div class="sub-tabs">
            <button class="sub-tab active" data-tab="blotter" onclick="switchTab('blotter', this)">
                ...
                Blotter Reports
                <span class="tab-badge">{{ $blotterCases->count() }}</span>
            </button>
            <button class="sub-tab" data-tab="business" onclick="switchTab('business', this)">
                <i class="fas fa-store"></i>
                Business Permits
                <span class="tab-badge">{{ $businesses->count() }}</span>
            </button>
        </div>

        {{-- Blotter tab --}}
        <div id="tab-blotter" class="tab-pane active">
            @if($blotterCases->isNotEmpty())
            <div class="sub-list">
                @foreach($blotterCases as $case)
                <a href="{{ route('portal.track') }}?ref={{ $case->case_number }}" class="sub-card">
                    <div class="sub-card-icon ic-blotter">
                        <i class="fas fa-gavel"></i>
                    </div>
                    <div class="sub-card-body">
                        <div class="sub-card-ref">{{ $case->case_number }}</div>
                    ...
                    <div class="sub-card-right">
                        ...
                        @endphp
                        <span class="sub-status {{ $statusClass }}">
                            {{ $case->status }}
                        </span>
            ...
            <div class="sub-empty">
                ...
                <p>No blotter reports were filed using <strong>{{ $contact }}</strong> as the contact number.</p>
                <a href="{{ route('portal.blotter') }}" class="btn btn-outline" style="margin-top:1.25rem;font-size:.85rem;min-height:40px">
        ...
        <div id="tab-business" class="tab-pane">
            ...
            <div class="sub-list">
                ...
                <a href="{{ route('portal.track') }}?ref={{ $biz->permit_number }}" class="sub-card">
                    <div class="sub-card-icon ic-business">
                        <i class="fas fa-store"></i>
                    </div>
                    <div class="sub-card-body">
                        <div class="sub-card-ref">{{ $biz->permit_number }}</div>
                        <div class="sub-card-title">{{ $biz->business_name }}</div>
                        <div class="sub-card-meta">
                    ...
                    <div class="sub-card-right">
                        ...
                        @endphp
                        <span class="sub-status {{ $bizStatusClass }}">
                            {{ $biz->status }}
                        </span>
                        <span class="sub-card-arrow"><i class="fas fa-chevron-right"></i></span>
                    </div>
            ...
            <div class="sub-empty">
                ...
                <p>No business permit applications were filed using <strong>{{ $contact }}</strong> as the contact number.</p>
                <a href="{{ route('portal.business') }}" class="btn btn-outline" style="margin-top:1.25rem;font-size:.85rem;min-height:40px">
    ...
    @if($blotterCases->isEmpty() && $businesses->isEmpty())
    <div class="p-alert p-alert-warning" style="margin-top:1rem">
        <i class="fas fa-triangle-exclamation"></i>
        <span>No submissions found for contact number <strong>{{ $contact }}</strong>. Please check the number and try again, or use the contact number you entered when submitting.</span>
    </div>
    @endif
