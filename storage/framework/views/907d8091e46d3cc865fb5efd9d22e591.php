<?php $__env->startSection('title', 'Portal Business Requests'); ?>
<?php $__env->startSection('page-title', 'Portal Business Requests'); ?>
<?php $__env->startSection('page-subtitle', 'Business permit requests submitted via the Resident Portal'); ?>
<?php $__env->startSection('content'); ?>

<div class="page-header">
    <div>
        <h1 class="page-title">Portal Business Permit Requests</h1>
        <p class="page-subtitle">Business permit requests submitted via the Resident Portal</p>
    </div>
...
<?php
    ...
    foreach($statuses as $s) {
        $counts[$s] = \App\Models\BusinessPermitRequest::where('status', $s)->count();
    }
    $total    = \App\Models\BusinessPermitRequest::count();
...
<div class="grid-4 mb-6">
    ...
    <div class="stat-card">
        <div class="stat-icon" style="background:rgba(37,99,235,0.08);color:#2563eb"><i class="fas fa-magnifying-glass"></i></div>
        <div class="stat-info">
            <div class="stat-number"><?php echo e(number_format($counts['Under Review'] ?? 0)); ?></div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-info">
            <div class="stat-number"><?php echo e(number_format($approved)); ?></div>
...
<div class="card mb-6">
    <div class="card-header" style="cursor:pointer" onclick="toggleFilters('pbiz')">
        ...
        <button type="button" class="btn btn-gold btn-sm" onclick="event.stopPropagation();toggleFilters('pbiz')">
                ...
                <div class="form-group" style="grid-column:1/-1">
                    ...
                    <div style="position:relative">
                        ...
                        <input type="text" id="searchInput" class="form-control" style="padding-left:32px"
                               placeholder="Owner name, business name, request number…">
                    </div>
                </div>
...
<div class="card">
    <div class="card-header">
        <span class="card-title"><i class="fas fa-store"></i> Business Permit Requests</span>
    </div>
    <div class="table-responsive">
        <table id="pbizTable" style="width:100%">
            <thead>
                <tr>
                    <th>Request No.</th>
                    <th>Owner</th>
    ...
    <div style="background:var(--surface);border-radius:var(--radius-lg);width:100%;max-width:440px;
        ...
        <div style="background:var(--navy);padding:16px 20px;display:flex;align-items:center;justify-content:space-between">
            <div style="display:flex;align-items:center;gap:10px">
                ...
                <span style="font-size:14px;font-weight:700;color:#fff">Update Business Request Status</span>
            </div>
            <button onclick="closePbizModal()"
        ...
        <div style="padding:20px">
            <p style="font-size:13px;color:var(--text-muted);margin-bottom:16px">
                Request: <strong id="pbizModalNum" style="color:var(--navy)"></strong>
            </p>
            <div class="form-group">
                <label class="form-label">New Status <span style="color:var(--crimson)">*</span></label>
                <select id="pbizModalStatus" class="form-control">
                    <?php $__currentLoopData = $statuses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($s); ?>"><?php echo e($s); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            ...
            <div class="form-group">
                <label class="form-label">Notes <span style="font-size:12px;color:var(--text-subtle);font-weight:400">(optional — visible to applicant if email provided)</span></label>
                <textarea id="pbizModalNotes" class="form-control" rows="3"
                          placeholder="e.g., Scheduled for inspection on…"></textarea>
            </div>
            <div id="pbizStatusError" style="display:none;font-size:13px;color:var(--crimson);
            ...
            <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:4px">
                <button type="button" onclick="closePbizModal()" class="btn btn-secondary">Cancel</button>
                <button type="button" id="pbizStatusSaveBtn" onclick="savePbizStatus()" class="btn btn-primary">
...
#pbizTable_wrapper .dataTables_length, #pbizTable_wrapper .dataTables_filter { display:none; }
#pbizTable_wrapper .dataTables_info { font-size:13px;color:var(--text-muted);padding:12px 20px; }
#pbizTable_wrapper .dataTables_paginate { padding:12px 20px; }
#pbizTable_wrapper .dataTables_paginate .paginate_button { padding:4px 10px;border-radius:6px;font-size:13px;cursor:pointer;border:1px solid var(--border) !important;background:white !important;color:var(--text) !important;margin:0 2px; }
#pbizTable_wrapper .dataTables_paginate .paginate_button.current { background:var(--navy) !important;color:white !important;border-color:var(--navy) !important; }
#pbizTable_wrapper .dataTables_paginate .paginate_button:hover:not(.current) { background:var(--navy-pale) !important;color:var(--navy) !important; }
...
$(document).ready(function () {
    ...
    var table = $('#pbizTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '<?php echo e(route('portal-business.index')); ?>',
            data: function (d) {
        ...
        columns: [
            ...
            { data: 'business_col',name: 'business_name', orderable: false },
        ...
        language: {
            ...
            emptyTable:  '<div class="empty-state"><i class="fas fa-store"></i><p>No business requests found.</p></div>',
            zeroRecords: '<div class="empty-state"><i class="fas fa-search"></i><p>No requests match your filters.</p></div>',
        }
    });

    $('#pbizTable').on('click', 'form[data-confirm] button[type="submit"]', function (e) {
    ...
    $('#pbizTable').on('click', '.pbiz-status-btn', function () {
        _pbizId = $(this).data('id');
        document.getElementById('pbizModalNum').textContent    = $(this).data('num');
        document.getElementById('pbizModalStatus').value       = $(this).data('status');
        document.getElementById('pbizModalNotes').value        = $(this).data('notes') || '';
        document.getElementById('pbizStatusError').style.display = 'none';
        document.getElementById('pbizStatusModal').style.display = 'flex';
    });
    ...
    const lsOpen = localStorage.getItem('fp_pbiz') === '1';
...
var _pbizId = null;
function closePbizModal() { document.getElementById('pbizStatusModal').style.display = 'none'; _pbizId = null; }
document.addEventListener('keydown', function (e) { if (e.key === 'Escape') closePbizModal(); });

function savePbizStatus() {
    if (!_pbizId) return;
    const btn    = document.getElementById('pbizStatusSaveBtn');
    const errDiv = document.getElementById('pbizStatusError');
    const status = document.getElementById('pbizModalStatus').value;
    const notes  = document.getElementById('pbizModalNotes').value;
    errDiv.style.display = 'none';
    btn.disabled  = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin" style="color:var(--gold)"></i> Saving…';
    axios.patch('/portal-business/' + _pbizId + '/status', { status: status, notes: notes })
        .then(function (res) {
            closePbizModal();
            bmsToast(res.data.message || 'Status updated.', 'success');
            $('#pbizTable').DataTable().ajax.reload(null, false);
        })
        .catch(function (err) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\anakco_bms\resources\views/portal-business/index.blade.php ENDPATH**/ ?>